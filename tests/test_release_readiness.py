...
def ready_inputs(evidence_dir: Path) -> dict:
    ...
    return {
        ...
        "release_control_ownership": {
            "status": "pass",
            "control_count": 70,
            "release_check_count": 70,
            "covered_release_check_count": 70,
            "tier0_count": 47,
            "every_release_count": 58,
            "owner_group_count": 5,
            "detected_fixture_count": 6,
            "failed_count": 0,
        },
        "control_traceability": {
            "status": "pass",
            "control_count": 65,
            "evidence_file_count": 131,
            "source_input_count": 68,
            "policy_input_count": 66,
            "test_file_count": 65,
            "detected_fixture_count": 6,
            "failed_count": 0,
        },
        ...
        "evidence_schema": {
            "status": "pass",
            "artifact_count": 25,
            "required_field_count": 287,
            "required_check_count": 169,
            "detected_fixture_count": 25,
            "failed_count": 0,
        },
        "validation_contract": {
            "status": "pass",
            "py_compile_script_count": 74,
            "generation_script_count": 72,
            "direct_validation_script_count": 71,
            "policy_json_count": 67,
            "committed_json_count": 82,
            "release_argument_count": 71,
            "detected_fixture_count": 6,
            "failed_count": 0,
        },
        ...
        "architecture_decisions": {
            ...
            "evidence_link_count": 18,
            "existing_evidence_link_count": 18,
            "release_control_count": 18,
            "detected_fixture_count": 7,
            "failed_count": 0,
        },
        "reviewer_reproducibility": {
            "status": "pass",
            "present_file_count": 8,
            "command_count": 6,
            "evidence_path_count": 6,
            "existing_evidence_path_count": 6,
            "boundary_term_count": 7,
            "release_control_count": 6,
            "detected_fixture_count": 6,
            "failed_count": 0,
        },
        "threat_model": {
            ...
            "present_file_count": 5,
            "asset_count": 6,
            "trust_boundary_count": 6,
            "threat_count": 6,
            "mitigation_count": 29,
            "owner_count": 6,
            "residual_risk_count": 6,
            "evidence_link_count": 19,
            "existing_evidence_link_count": 19,
            "release_control_count": 19,
            "detected_fixture_count": 7,
            "failed_count": 0,
        },
        "data_handling": {
            ...
            "present_file_count": 6,
            "data_class_count": 6,
            "handling_term_count": 54,
            "retention_term_count": 6,
            "owner_count": 6,
            "evidence_link_count": 18,
            "existing_evidence_link_count": 18,
            "release_control_count": 18,
        ...
        "public_claim_evidence": {
            "status": "pass",
            "claim_count": 19,
            "evidence_claim_count": 19,
            "release_check_count": 19,
        ...
        "maintainer_intake": {
            "status": "pass",
            "present_file_count": 6,
            "issue_template_count": 2,
            "pr_validation_command_count": 3,
            "support_term_count": 5,
            "detected_fixture_count": 6,
            "failed_count": 0,
        },
        "release_notes_contract": {
            "status": "pass",
            "release_note_field_count": 4,
        ...
        "evidence_provenance": {
            "status": "pass",
            "artifact_count": 157,
            "source_input_count": 169,
            "failed_count": 0,
        },
        "proof_packet_integrity": {
            "status": "pass",
            "manifest_entry_count": 330,
            "evidence_artifact_count": 157,
            "generated_artifact_count": 4,
            "source_input_count": 169,
            "matched_digest_count": 330,
...
class ReleaseReadinessTest(unittest.TestCase):
    ...
    def test_requires_architecture_decision_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["architecture_decisions"]["accepted_decision_count"] = 3
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["architecture_decision_audit"])

    def test_requires_reviewer_reproducibility_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["reviewer_reproducibility"]["command_count"] = 2
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["reviewer_reproducibility_audit"])

    def test_requires_threat_model_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["threat_model"]["threat_count"] = 2
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["threat_model_audit"])

    def test_requires_data_handling_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["data_handling"]["data_class_count"] = 2
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["data_handling_audit"])

    def test_requires_dependency_update_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["dependency_update"]["weekly_schedule_count"] = 0
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["dependency_update_audit"])

    def test_requires_security_scanning_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["security_scanning"]["language_count"] = 0
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["security_scanning_audit"])

    def test_requires_maintainer_intake_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["maintainer_intake"]["issue_template_count"] = 1
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["maintainer_intake_audit"])

    def test_requires_release_notes_contract_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["release_notes_contract"]["evidence_reference_count"] = 2
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["release_notes_contract_audit"])
    ...
    def test_requires_validation_contract_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            inputs = ready_inputs(Path(tmp))

            inputs["validation_contract"]["committed_json_count"] = 12
            report = release_readiness.evaluate(**inputs)
            checks = {item["name"]: item["ok"] for item in report["checks"]}

            self.assertEqual("fail", report["status"])
            self.assertFalse(checks["validation_contract_audit"])
