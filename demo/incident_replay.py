...
SCENARIOS: list[dict[str, Any]] = [
    {
        ...
        "model_variant": "recommender-small",
        "status_code": 200,
        "error_rate": 0.0,
        "service_version": "v1",
...
def build_scenario_payload(scenario: dict[str, Any], base_ns: int) -> dict[str, object]:
    ...
    for index, latency_ms in enumerate(latencies):
        ...
        root_attrs = [
            ...
            attribute("ai.model.variant", str(scenario["model_variant"])),
            attribute("ai.inference.latency_ms", int(latency_ms)),
            attribute("ai.prompt.tokens", int(scenario["input_tokens"])),
            attribute("ai.completion.tokens", int(scenario["output_tokens"])),
            attribute("ai.gpu.compute_ms", int(scenario["gpu_ms"])),
            attribute("ai.queue.wait_ms", int(scenario["queue_ms"])),
            ...
            attribute("tenant.tier", str(scenario["tenant_tier"])),
            attribute("request.priority", str(scenario["request_priority"])),
        ...
        spans.append(
            span(
                ...
                duration_ms=int(scenario["vector_cache_ms"]) if cache_result == "miss" else 6,
                attrs=[
                    attribute("cache.result", str(cache_result)),
                    attribute("incident.scenario", str(scenario["name"])),
        ...
        spans.append(
            span(
                ...
                duration_ms=int(scenario["model_ms"]),
                attrs=[
                    attribute("ai.model", "demo-recommender"),
                    attribute("ai.model.variant", str(scenario["model_variant"])),
                    attribute("ai.gpu.compute_ms", int(scenario["gpu_ms"])),
...
def summarize_scenario(scenario: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "vector_cache_ms": scenario["vector_cache_ms"],
        "dependency_ms": scenario["dependency_ms"],
        "model_ms": scenario["model_ms"],
        "queue_ms": scenario["queue_ms"],
        "gpu_ms": scenario["gpu_ms"],
        "input_tokens": scenario["input_tokens"],
        "output_tokens": scenario["output_tokens"],
        "tenant_tier": scenario["tenant_tier"],
        "request_priority": scenario["request_priority"],
